...
Приложение использует **YandexGPT 5.1 Pro** для текстовых ответов и **YandexART** для генерации изображений аватаров экспертов.

### Вариант 1: Использование Yandex Cloud (рекомендуется)

1. Зарегистрируйтесь на [Yandex Cloud](https://cloud.yandex.ru/)

2. Создайте каталог (folder) в облаке

   **Если при создании каталога возникает ошибка "Доступ запрещен":**
   
   - **Решение 1:** Убедитесь, что вы зарегистрированы и подтвердили свой аккаунт
   - **Решение 2:** Проверьте, что у вас есть права на создание каталогов в организации
   - **Решение 3:** Попробуйте создать каталог через веб-интерфейс Yandex Cloud Console
   - **Решение 4:** Если вы в организации, обратитесь к администратору для получения прав
   - **Решение 5:** Используйте бесплатный пробный период - создайте новый аккаунт Yandex Cloud
   - **Решение 6:** Проверьте, что вы находитесь в правильном облаке (личное/организационное)
   - **Альтернатива:** Можно использовать существующий каталог, если у вас есть доступ к нему
...
4. Получите Folder ID:
   - В консоли Yandex Cloud найдите ваш каталог
   - Скопируйте ID каталога (обычно выглядит как `b1gxxxxxxxxxxxxxxxx`)

### Вариант 2: Использование Yandex AI Studio напрямую

1. Перейдите на [Yandex AI Studio](https://ai.yandex.ru/)
2. Зарегистрируйтесь или войдите в аккаунт
3. Создайте проект
4. Получите API ключ в настройках проекта
5. Используйте ID проекта как Folder ID

**Примечание:** YandexGPT 5.1 Pro доступен через AI Studio и требует указания модели `yandexgpt-pro/latest` в настройках.
...
# Опционально: укажите модель (по умолчанию yandexgpt-pro/latest)
...
# OAuth (опционально, для входа через VK и Google)
VK_APP_ID=ваш-vk-app-id
VK_APP_SECRET=ваш-vk-app-secret
...
3. Получите ID приложения и защищенный ключ:
   - В настройках приложения найдите "ID приложения" (это ваш `VK_APP_ID`)
   - В разделе "Настройки" → "Безопасность" найдите "Защищенный ключ" (это ваш `VK_APP_SECRET`)

4. Настройте URL для OAuth:
   - В настройках приложения перейдите в раздел "Настройки"
   - В поле "Адрес сайта" укажите: `http://localhost:5000` (для разработки)
   - В поле "Доверенный redirect URI" добавьте: `http://localhost:5000/oauth/vk/callback`
...
**Для VK:**
- Проверьте, что URL в настройках приложения совпадает с вашим доменом
