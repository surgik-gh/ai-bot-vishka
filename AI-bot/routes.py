...
from models import User, Expert, Subject, Lesson, Quiz, Question, UserAnswer, Achievement, UserAchievement, TokenTransaction, EmailVerificationCode
...
def create_lesson():
    try:
        ...
        if not Config.GEMINI_API_KEY:
            return jsonify({'success': False, 'message': 'Gemini API не настроен. Обратитесь к администратору.'}), 500
        ...
        # Analyze material
        explanation = gemini_api.analyze_material(material_text, expert_prompt)
        if not explanation:
        ...
        questions_data = gemini_api.generate_quiz(material_text, explanation, expert_prompt, num_questions)
...
def submit_quiz():
    ...
    summary = gemini_api.generate_lesson_summary(
...
def admin_experts():
    ...
    if request.method == 'POST':
        ...
        expert_prompt, avatar_description, avatar_image_base64 = gemini_api.generate_expert(description)
